...
for i in sorted(data_path.rglob('francesco_green.npy'))[::-1]:
    ...
    np.savetxt(save_folder / 'stim_frame_indicies.txt', this_stim_volume_inds, fmt='%i',delimiter=',')
    np.savetxt(save_folder / 'stim_cell_indicies.txt', this_stim_cell_ids, fmt='%i', delimiter=',')

    this_cell_ids = [[i] for i in this_cell_ids]
    import csv
    with open(save_folder / 'cell_ids.txt', 'w') as f:
        w = csv.writer(f)
        w.writerows(this_cell_ids)
